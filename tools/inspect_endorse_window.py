"""
Run this script while the AMS360 Endorse window is open on screen.

Modes:
  python tools/inspect_endorse_window.py              → full dump to logs/endorse_window_controls.txt
  python tools/inspect_endorse_window.py combo        → list all ComboBox values in the window
  python tools/inspect_endorse_window.py search <kw>  → search all controls whose title/auto_id contains keyword
"""
...
OUTPUT_FILE     = "logs/endorse_window_controls.txt"
# TODO: confirm exact window title once the Endorse window is inspected live.
WINDOW_TITLE_RE = "Endorsement"
...
def list_combo_values(win):
    """List all ComboBox controls and their items found anywhere in the Endorse window."""
    print("=== All ComboBoxes in Endorse window ===\n")
    combos = win.descendants(control_type="ComboBox")
    if not combos:
        print("No ComboBox found in the Endorse window.")
        return
    for combo in combos:
        try:
...
def main():
    ...
    if mode == "combo":
        ...
        if len(sys.argv) < 3:
            print("Usage: python tools/inspect_endorse_window.py search <keyword>")
            print("Example: python tools/inspect_endorse_window.py search EffectiveDate")
        else:
